"""Parse language-agnostic firmware identity lines (throwaway).

Expected product shape (one line, space-separated key=value tokens)::

    fw_name=XUSSC fw_version=0.0.1

Also accepts MicroPython-style two-line dumps if a host ever prints::
...
# Bare token that looks like a semver-ish version (digits and dots).
...
def parse_identity_blob(text: str) -> Identity | None:
    """Find the best identity in a serial capture blob.

    Prefers a key=value line containing fw_name and/or fw_version.
    Falls back to two consecutive non-empty lines (name, version).
    """
    if not text:
        return None

    # Prefer key=value lines (product protocol / boot banner).
    ...
    # Two-line dump: NAME\nVERSION (deploy --verify MicroPython path style)
...
def match_expected(
    ...
    *,
    ...
    """Return human lines; empty list means OK (or nothing expected)."""
    fails: list[str] = []
