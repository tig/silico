...
| Deploy overwrite | Inspect first; write only with operator yes (`silico deploy --port COMx --yes`). MicroPython: usually `[deploy].core` files (live `PROGRESS [write] i/n`). **C / ESP-IDF:** full app image (`idf.py build` + flash). **Announce** post-boot product face (sound/light/motion) before write/reset |
